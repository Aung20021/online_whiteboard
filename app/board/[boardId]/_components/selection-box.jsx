...
export const SelectionBox = memo(({ onResizeHandlePointerDown, }) => {
    const soleLayerId = useSelf((me) => me.presence.selection.length === 1 ? me.presence.selection[0] : null);
    const isShowingHandles = useStorage((root) => soleLayerId && ![LayerType.Path, LayerType.Connector].includes(root.layers.get(soleLayerId)?.type));
    const bounds = useSelectionBounds();
    if (!bounds) {
        return null;
    }
    return (<>
      <rect className="fill-transparent stroke-blue-500 stroke-1 pointer-events-none" style={{
